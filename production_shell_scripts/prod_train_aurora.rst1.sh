...
PWD="/flare/FoundEpidem/avasan/IDEAL/Diffusion/diffusion_training/protein_lig_sedd"

# Job Configuration
JOB_NAME="aurora_sedd_ddp"
QUEUE="prod"                # Aurora queue name
WALLTIME="06:00:00"          # 2 hours
PPN=12                        # Processes per node (should match NUM_GPUS)
NUM_GPUS=$((NODES * PPN))                    # Number of CUDA devices to use

# Environment
VENV_PATH="/lus/flare/projects/FoundEpidem/avasan/IDEAL/Diffusion/protein_lig_sedd/pldd_venv"        # Virtual environment path
...
if [ "$EXECUTION_MODE" = "interactive" ]; then
    ...
    mpirun -np $NUM_GPUS -ppn $PPN \
        ...
        --device "$DEVICE" \
        --seed $SEED \
        $FRESH_FLAG 2>&1 | tee "$LOG_DIR/aurora_ddp_interactive_nodes${NODES}.log"

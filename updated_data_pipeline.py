...
class ImprovedProteinLigandDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        # Extract basic data
        protein_tokens = torch.tensor(sample['protein_tokens'], dtype=torch.long)
        ligand_tokens = torch.tensor(sample['ligand_tokens'], dtype=torch.long)
...
def improved_protein_ligand_collate_fn(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for i, sample in enumerate(batch):
        ...
        protein_tokens[i, :protein_len] = sample['protein_tokens'][:protein_len]
        protein_mask[i, :protein_len] = sample['protein_mask'][:protein_len]
        
        ligand_tokens[i, :ligand_len] = sample['ligand_tokens'][:ligand_len]
...
def create_improved_data_loaders(
    plinder_output_dir: Optional[Union[str, Path]] = None,
    ...
    if plinder_output_dir is None:
        plinder_output_dir = "./processed_plinder"
    
    plinder_output_dir = Path(plinder_output_dir)
    data_file = plinder_output_dir / "processed_plinder_data.pt"
    
    # Check if we need to process data
    if force_reprocess or not data_file.exists():
        logger.info("Processing PLINDER data...")
        processor = ImprovedPLINDERProcessor(
            output_dir=plinder_output_dir,
            cache_dir=plinder_output_dir / "cache",
            plinder_data_dir=plinder_data_dir
        )
        
        # Process dataset
        data_file_path = processor.process_dataset(
            max_samples=max_samples,
            num_workers=1  # Use single worker for stability
        )
        data_file = Path(data_file_path)
    else:
        logger.info(f"Using existing processed data: {data_file}")
...
def test_improved_data_pipeline():
    ...
    train_loader, val_loader, test_loader = create_improved_data_loaders(
        plinder_output_dir="./test_improved_pipeline",
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        logger.info(f"  Protein tokens shape: {batch['protein_tokens'].shape}")
        logger.info(f"  Ligand tokens shape: {batch['ligand_tokens'].shape}")

...
class DualTrackDiffusionTransformer(nn.Module):
    ...
    def __init__(self, config, vocab_size_protein, vocab_size_ligand):
        ...
        self.protein_embed = ImprovedEmbeddingLayer(config.model.hidden_size, vocab_size_protein)
        ...
        self.rotary_emb_prot = rotary.Rotary(config.model.hidden_size // config.model.n_heads)
        self.rotary_emb_lig = rotary.Rotary(config.model.hidden_size // config.model.n_heads)
...
class ProteinLigandDiffusionModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Get vocab sizes
        vocab_size_protein = config.data.vocab_size_protein + (1 if config.graph.type == "absorb" else 0)
        vocab_size_ligand = config.data.vocab_size_ligand + (1 if config.graph.type == "absorb" else 0)

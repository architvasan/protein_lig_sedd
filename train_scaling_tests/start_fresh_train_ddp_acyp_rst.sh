...
SEED=42
CHECKPOINT_FILE="experiments/polaris_ddp_2nodes20250928_162954/checkpoints/best_checkpoint.pth"
...
CHECKPOINT_ACTION="restart" # Options: "remove", "backup", "ignore", "cancel", "restart"
                            ...
                            # restart: Continue with existing checkpoint
...
if [ -d "$WORK_DIR/checkpoints" ] && [ "$(ls -A $WORK_DIR/checkpoints)" ]; then
    ...
    case $CHECKPOINT_ACTION in
        ...
        "ignore")
            ...
            ;;
        "restart")
            echo "🚀 Restarting from existing checkpoint $CHECKPOINT_FILE"
            ;;
        "cancel")
...
if [ "$EXECUTION_MODE" = "interactive" ]; then
    ...
    mpirun -np $NUM_GPUS -ppn $PPN \
        ...
        2>&1 | tee "$LOG_DIR/acyp_polaris_ddp_interactive_nodes${NODES}.log"
